...
int Board::operator()(int i, int j) const
{
    if (i < 0 || j < 0 || i >= N || j >= N)  // si las coordenadas están fuera del tablero
        return out_of_board;
    if (!is_it_black_field(i, j))  // si las coordenadas no están en una casilla negra
        return out_of_board;
    return board[N / 2 * i + j / 2];  // acceso a la casilla en la matriz interna
}
...
ostream& operator<<(ostream& out, const Board& p)  // Cambiado a const Board&
{
    HANDLE hconsole = GetStdHandle(STD_OUTPUT_HANDLE);
    cout << endl << endl;
    for (int i = (turn_board) ? 0 : N - 1; i >= 0 && i < N; i += (turn_board) ? 1 : -1)
    {
        SetConsoleTextAttribute(hconsole, 15); // Reset settings
